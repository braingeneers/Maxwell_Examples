...
struct SpikeEvent
{
    ...
    static const unsigned int bSize = sizeof(unsigned long) +
                                      sizeof(int) +
                                      sizeof(float);
    ...
    SpikeEvent(unsigned char *buffer )
        : frameNo(*reinterpret_cast<unsigned long int*>(&buffer[0])),
        channel(*reinterpret_cast<int*>(&buffer[sizeof(frameNo)])),
        amp(*reinterpret_cast<float*>(&buffer[sizeof(frameNo)+sizeof(channel)]))
    { }
};
...
void dataRead(int detection_channel) {
    ...
    // Connect to API spike data production port.
    subscriber.connect("tcp://localhost:7205"); // was 7780

    // Socket to communicate with the API
    std::string connection = "tcp://localhost:6022";
    ...
    while (true) {
        ...
        // Dump every buffer to the terminal! :D
        unsigned int *d = (unsigned int *)events.data();
        ...
        for (int i=0;i<spikes.size();i++) {
            /*
            if (spikes[i].amp != 0)
                std::cerr << spikes[i].frameNo << " " << i << 
                    ": spike on " << spikes[i].channel << 
                    " with amplitude " << float(spikes[i].amp) << std::endl;
                    */

            if (spikes[i].channel==detection_channel && blanking==0)
            {
