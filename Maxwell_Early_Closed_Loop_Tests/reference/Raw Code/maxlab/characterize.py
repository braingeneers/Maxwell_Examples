...
def binary_search_code(readout_channel, target, min_code=0, max_code=1024, sleeptime=1.2, debug=True):
        span = (max_code - min_code)/2
        last_code = (max_code - min_code)/2
...
class StimulationUnitCharacterizer(object):
    ...
    def connect_stimulation_unit_to_external_port(self, unit_no):
        ...
        stim_unit.power_up(True).connect(True).set_current_mode().dac_source(0)
    ...
    def characterize(self, unit_no):
        ...
        self.connect_stimulation_unit_to_external_port(unit_no)
